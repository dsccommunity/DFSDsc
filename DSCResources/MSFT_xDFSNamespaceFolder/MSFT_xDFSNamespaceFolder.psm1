...
if ($Ensure -eq 'Present')
{
    ...
    if
            if (($TimeToLiveSec) `)
    {
        Write-Verbose.TimeToLiveSec -ne $TimeToLiveSec))
                "$($MyInvocation.MyCommand): "
                $(Verbose -Message ( @(
                    "$($MyInvocation.MyCommand): "
        )($LocalizedData.NamespaceFolderParameterNeedsUpdateMessage) `
    $desiredConfigurationMatchth,'TimeToLiveSec'
}       ) -join '' )
                $desiredConfigurationMatch = $false
if
)
{
    Write-VerbosenableInsiteReferrals) `
            "$($MyInvocation.MyCommand): "site Referrals') -ne $EnableInsiteReferrals))
            $(                             {
                Write-Verbose -Message ( @(
    )$($MyInvocation.MyCommand): "
$desiredConfigurationMatchpaceFolderParameterNeedsUpdateMessage) `
}           -f $Path,$TargetPath,'EnableInsiteReferrals'
                    ) -join '' )
if  $desiredConfigurationMatch = $false
            })
{
    Write-Verbose
            "$($MyInvocation.MyCommand): "`
            $(($Folder.Flags -contains 'Target Failback') -ne $EnableTargetFailback))
                                                  {
    )-Verbose -Message ( @(
$desiredConfigurationMatchmand): "
}       $($LocalizedData.NamespaceFolderParameterNeedsUpdateMessage) `
...
ifarget = Get-FolderTarget `
{   -Path $Path `
    ifargetPath $TargetPath
)
    {
        Write-Verbose
                "$($MyInvocation.MyCommand): "
...
if          -f $Path,$TargetPath,'ReferralPriorityClass'
                        ) -join '' ))
{
    Write-VerbosegurationMatch = $false
            "$($MyInvocation.MyCommand): "
            $(
                if (($ReferralPriorityRank) `
    )($Target.ReferralPriorityRank -ne $ReferralPriorityRank))
$desiredConfigurationMatch
}   Write-Verbose -Message ( @(
}           "$($MyInvocation.MyCommand): "
else        $($LocalizedData.NamespaceFolderTargetParameterNeedsUpdateMessage) `
{               -f $Path,$TargetPath,'ReferralPriorityRank'
    # The Folder target does not exist but should - change required
    Write-VerboseonfigurationMatch = $false
            "$($MyInvocation.MyCommand): "
            $(
            else
    )
$desiredConfigurationMatchot exist but should - change required
}   Write-Verbose -Message ( @(
}           "$($MyInvocation.MyCommand): "
else        $($LocalizedData.NamespaceFolderTargetDoesNotExistButShouldMessage) `
{               -f $Path,$TargetPath
...
$desiredConfigurationMatchsn't exist but should - change required
}   Write-Verbose -Message ( @(
}           "$($MyInvocation.MyCommand): "
else         $($LocalizedData.NamespaceFolderDoesNotExistButShouldMessage) `
{               -f $Path,$TargetPath
    ...
    else
    {
        ...
        $desiredConfigurationMatch but should not - change required
    }   Write-Verbose -Message ( @(
    else    "$($MyInvocation.MyCommand): "
...
{           $($LocalizedData.NamespaceFolderTargetDoesNotExistAndShouldNotMessage) `
    ...
    Write-Verbosein '' )
            ...
            $(
                ...
                "$($MyInvocation.MyCommand): "
return       $($LocalizedData.NamespaceFolderDoesNotExistAndShouldNotMessage) `
                    -f $Path,$TargetPath
                ) -join '' )
        }
    } # if
